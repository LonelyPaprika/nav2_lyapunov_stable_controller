...
#include "nav2_stable_controller/stable_controller.hpp"
...
void StableController::configure(const rclcpp_lifecycle::LifecycleNode::WeakPtr& parent, std::string name,
...
void StableController::activate() {
    RCLCPP_INFO(logger_, "Activating controller: %s of type stable_controller::StableController\"  %s",
                plugin_name_.c_str(), plugin_name_.c_str());
    global_pub_->on_activate();
}

void StableController::deactivate() {
    RCLCPP_INFO(logger_, "Dectivating controller: %s of type stable_controller::StableController\"  %s",
                plugin_name_.c_str(), plugin_name_.c_str());
    global_pub_->on_deactivate();
}

void StableController::cleanup() {
    RCLCPP_INFO(logger_, "Cleaning up controller: %s of type stable_controller::StableController",
                plugin_name_.c_str());
    global_pub_.reset();
}

void StableController::setPlan(const nav_msgs::msg::Path& path) {
    global_pub_->publish(path);
    global_plan_ = path;
}

void StableController::setSpeedLimit(const double& speed_limit, const bool& percentage) {
    (void)speed_limit;
    (void)percentage;
}
...
geometry_msgs::msg::TwistStamped StableController::computeVelocityCommands(
...
geometry_msgs::msg::Pose StableController::selectGoal(const nav_msgs::msg::Path& transformed_plan) {
...
CmdVelocity StableController::computeVelocity(const geometry_msgs::msg::Pose& goal_pose) {
...
void StableController::checkCollision(const geometry_msgs::msg::PoseStamped& pose, const CmdVelocity& cmd) {
    if (use_collision_detection_ && isCollisionImminent(pose, cmd.linear_vel, cmd.angular_vel, 1.0)) {
        throw nav2_core::PlannerException("StableController detected collision ahead!");
    }
}

geometry_msgs::msg::TwistStamped StableController::generateTwistMsg(
...
nav_msgs::msg::Path StableController::transformGlobalPlan(const geometry_msgs::msg::PoseStamped& pose) {
...
bool StableController::transformPose(const std::shared_ptr<tf2_ros::Buffer> tf, const std::string frame,
...
bool StableController::isCollisionImminent(const geometry_msgs::msg::PoseStamped& robot_pose,
...
bool StableController::inCollision(const double& x, const double& y, const double& theta) {
    unsigned int mx, my;
...
PLUGINLIB_EXPORT_CLASS(nav2_stable_controller::StableController, nav2_core::Controller)